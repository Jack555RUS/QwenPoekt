  ...
  function loop(links, ul) {
    $.each(links, function(key,   obj) {
      ...
      if(obj['link'] == 'null'){
        ...
        var $relative_path_to_manual = document.head.querySelector("[property~=relative-path-to-manual][content]").content;
        var $a = $('<a>').attr('href', $relative_path_to_manual + obj['link'] + '.html').text(obj['title']);
...
****************************************/
  ...
  // accomodate both common syntax highlight conventions used in markdown by mapping everything to "lang-code" used by prism highlighter
  const classListToCheck = Object.keys(syntaxHighlightClassMappings).join(','); // Creates a comma-separated string of code classes
  document.querySelectorAll(`code[class*="${classListToCheck.split(',').join('"],code[class*="')}"]`)
     .forEach(replaceClasses);
...
function addCopyButtonsToCodeBlocks() {
    ...
    // Label used by accessibility technologies, this should describe the purpose of the control:
    const ariaLabel = "Copy code sample to the clipboard";
...
////////////////////////////////////////////////////////////////////////////
// Rationalize syntax highlight language codes
//
// Package docs and core docs use different markdown engines and syntax
// highlighters which have different ways of handling the langauge code
// assigned to fenced code blocks. This function alters the class assigned to
// such code blocks so that highlighting works with either form of code.
// This makes core manual markdown more compatible with package manual markdown.
//
// The first entry is the package code with no "lang-" prefix, the second entry
// is the prism highlighter code, with prefix.
const syntaxHighlightClassMappings = {
  'cs': 'lang-cs',
  'csharp': 'lang-cs',
  'C#': 'lang-cs',
  'c#': 'lang-cs',
  ...
  'plain': 'lang-plaintext',
  'plaintext': 'lang-plaintext',
  'text': 'lang-plaintext',
  'gradle': 'lang-gradle',
  'apacheconf': 'lang-apacheconf',
  'apache': 'lang-apache',
  'nginx': 'lang-nginx',
  'shell': 'lang-shell',
  'bash': 'lang-bash',
...
function replaceClasses(element) {
  for (const simpleClass in syntaxHighlightClassMappings) {
    if (element.classList.contains(simpleClass)) {
      element.classList.replace(simpleClass, syntaxHighlightClassMappings[simpleClass]);
      Prism.highlightElement(element); // Prism has already run, so call it again for this element.
    }
  }
}
...
function populateOtherVersionsContainer(versions){
  ...
  var $version_switcher_versions_with_this_page = document.head.querySelector("[property~=version-switcher-versions-with-this-page][content]").content;
  var $version_switcher_versions_without_this_page = document.head.querySelector("[property~=version-switcher-versions-without-this-page][content]").content;
  var $version_switcher_supported = document.head.querySelector("[property~=version-switcher-supported][content]").content;
  var $version_switcher_legacy = document.head.querySelector("[property~=version-switcher-legacy][content]").content;

  // Create two divs, one for 'with this page', one without

  let versionsWithThisPageDiv = $('<div id="versionsWithThisPage"><li class="vsDivHeader"><p class="vsDivHeader"><b>' + $version_switcher_versions_with_this_page +'</b></p></li></div>').appendTo(otherVersionsContentUL);
  let versionsWithoutThisPageDiv = $('<div id="versionsWithoutThisPage"><li class="vsDivHeader"><p class="vsDivHeader"><b>' + $version_switcher_versions_without_this_page + '</b></p></li></div>').appendTo(otherVersionsContentUL);
  ...
  versions.supported.forEach(supportedVersion => {
    if(isVersionDifferent(currentVersionStringOrName, supportedVersion)){
      ...
      let supportedVersionElementMobile = $('<option id="' + supportedVersion.major + '-' + supportedVersion.minor + '" value="">' + linkText + ' - ' + $version_switcher_supported + '</option>').appendTo(versionsWithoutThisPageMobile);
  ...
  versions.notSupported.forEach(notSupportedVersion => {
    if(isVersionDifferent(currentVersionStringOrName, notSupportedVersion)){
      ...
      let notSupportedVersionElementMobile = $('<option id="' + notSupportedVersion.major + '-' + notSupportedVersion.minor + '" value="">' + linkText + ' - ' + $version_switcher_legacy + '</option>').appendTo(versionsWithoutThisPageMobile);
...
function getCurrentOpenPage(){
  ...
  if (urlObject.pathname.endsWith('30_search.html'))
  {
    // For the search page we need to retain the search query
    const page = urlObject.pathname.split('/Documentation/')[1];
    return page + urlObject.search;
  }
  else if (urlObject.pathname.includes('/Documentation/'))
  {
    // Versioned URL for example:
    // https://docs.hq.unity3d.com/6000.0/Documentation/Manual/urp/shaders-in-universalrp.html
    // Return everything after '/Documentation/' for example:
    // Manual/urp/shaders-in-universalrp.html
    return urlObject.pathname.split('/Documentation/')[1];
  }
  else
  {
    // Unversioned URL for example:
    // https://docs.hq.unity3d.com/Manual/urp/shaders-in-universalrp.html
    // Return everything after the domain, for example:
    // Manual/urp/shaders-in-universalrp.html
