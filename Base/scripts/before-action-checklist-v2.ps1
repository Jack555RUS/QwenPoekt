...
# ============================================
# ШАГ 0: ПРОВЕРКА ДОСТАТОЧНОСТИ ЗНАНИЙ
# ============================================

Write-Host "`n[0/7] Проверка достаточности знаний..." -ForegroundColor Cyan
...
📋 ВОПРОСЫ:
  1. Знаете ли вы достаточно для этой задачи?
  2. Все ли аспекты охвачены?
  3. Изучены ли лучшие практики?
  4. Есть ли 3+ варианта решения?
  5. Оценены ли риски?

Если "НЕТ" хотя бы на один:
  1. Изучить документацию (20 минут минимум)
  2. Найти лучшие практики (3+ источника)
  3. Предложить 3+ варианта решения
  4. Оценить риски
"@

Write-Host $knowledgeCheck -ForegroundColor Yellow

if (-not $Force) {
    $knowledgeConfirm = Read-Host "  Знаний достаточно? (Y/N)"
    if ($knowledgeConfirm -ne 'Y' -and $knowledgeConfirm -ne 'y') {
        Write-Host "`n📖 Вернитесь после изучения" -ForegroundColor Yellow
        Write-Log "НЕДОСТАТОЧНО ЗНАНИЙ — отложено" $Yellow
        exit 0
    }
}

Write-Host "  ✅ Проверка знаний завершена`n" -ForegroundColor Green
Write-Log "Проверка знаний: подтверждено"
