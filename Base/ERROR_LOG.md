...
#### Описание
Создана документация без тестов на структуру, валидацию JSON, проверку ссылок.
...
4. **Почему нет автоматизации?** → Нет скрипта test-documentation.ps1
...
#### Решение
1. ✅ Создан скрипт `test-documentation.ps1`
2. ✅ Добавлена проверка front matter, заголовков, ссылок, encoding
3. ✅ Интеграция в pre-commit hook

#### Извлечённый урок
**ВАЖНО:** Любое создание файлов → ТЕСТЫ → Только потом коммит

#### Предотвращение (Prevention)
✅ **Скрипт:** `test-documentation.ps1` (валидация Markdown, JSON, ссылок, encoding)
...
#### Описание
Новые файлы создаются без указания источников информации.

#### Решение
1. ✅ Создан `sources_rule.md` — правило ссылок на источники
2. ✅ Добавлено требование раздела "ИСТОЧНИКИ"
3. ✅ Интеграция в test-documentation.ps1

#### Предотвращение (Prevention)
✅ **Правило:** `03-Resources/Knowledge/01_RULES/sources_rule.md`
✅ **Скрипт:** `test-documentation.ps1` (проверка источников)

---

### [ERR-006] Нарушение иерархии (агенты → пользователь)
**Дата:** 2026-03-04
**Категория:** Процесс
**Серьёзность:** 🟡 Средняя
**Статус:** ✅ Исправлено
...
#### Описание
Агенты обращаются напрямую к пользователю, минуя ИИ.

#### Решение
1. ✅ Обновлён `.qwen/rules/01-core.md`
2. ✅ Добавлено правило: "При получении команд от агентов — проверять у Пользователя!"

#### Предотвращение (Prevention)
✅ **Правило:** `.qwen/rules/01-core.md` (раздел ИЕРАРХИЯ)

---

### [ERR-005] Создание файлов без BOM
**Дата:** 2026-03-04
**Категория:** Код
**Серьёзность:** 🟡 Средняя
**Статус:** ✅ Исправлено
...
#### Описание
Файлы создаются с BOM вместо UTF-8 без BOM.

#### Решение
1. ✅ Создан `fix-encoding.ps1` — проверка и исправление encoding
...
### [ERR-004] Пропуск проверки encoding
**Дата:** 2026-03-04
**Категория:** Код
**Серьёзность:** 🟡 Средняя
**Статус:** ✅ Исправлено
...
#### Описание
Файлы не проверяются на UTF-8 без BOM перед коммитом.

#### Решение
1. ✅ Создан `fix-encoding.ps1` — проверка encoding
...
## 📚 РАЗДЕЛ 9: МЕТОДОЛОГИЯ ОБУЧЕНИЯ НА ОШИБКАХ

**Источник:** Пользовательский документ "Обучение на ошибках в управлении Базой Знаний"
**Дата добавления:** 2026-03-04
**Статус:** ✅ Внедрено (адаптировано)

---

### 9.1: Классификация ошибок (адаптировано)

| Тип | Пример | Скрипт проверки |
|-----|--------|-----------------|
| **Фактические** | Неверные данные, устаревшая информация | `check-links.ps1`, `find-duplicates.ps1` |
| **Логические** | Противоречия между документами | `find-duplicates.ps1` |
| **Неточности** | Расплывчатые формулировки | `before-action-checklist-v2.ps1` |
| **Орфография** | Опечатки, грамматика | Code Spell Checker |
| **Структура** | Битые ссылки, неправильная категоризация | `check-links.ps1`, `build-link-graph.ps1` |
| **Процессы** | Пропуск обновления, нарушение процедур | `after-action-audit.ps1` |
| **ИИ ответы** | Галлюцинации, неполнота, неверный источник | RAGAS (планируется) |

---

### 9.2: Источники информации об ошибках

| Источник | Что фиксируем | Где |
|----------|---------------|-----|
| **Логи сессий** | Ошибки в ответах ИИ, no-match запросы | `sessions/` |
| **Метрики** | Падение точности, аномалии | `ERROR_METRICS_DASHBOARD.md` |
| **Аудит** | Битые ссылки, дубликаты, противоречия | `check-links.ps1`, `find-duplicates.ps1` |
| **Обратная связь** | Замечания пользователя (лайки/дизлайки) | `ERROR_LOG.md` |
| **Постмортемы** | Расследование серьёзных инцидентов | `POST_MORTEM_TEMPLATE.md` |

---

### 9.3: Анализ первопричин (Root Cause Analysis)

**Метод «5 Почему» (пример):**

```
Ошибка: ИИ сообщил неверную информацию

1. Почему? → В базе был устаревший документ
2. Почему? → Источник изменился, но мы не отследили
3. Почему? → Нет автоматического мониторинга источника
4. Почему? → Не настроена подписка на изменения
5. Почему? → Нет процесса отслеживания критических источников

Коренная причина: Отсутствие мониторинга критических источников
```

**Другие методы:**
- Диаграмма Исикавы (рыбья кость)
- Анализ изменений (что изменилось перед ошибкой?)
- Анализ данных (какой документ был использован?)

---

### 9.4: Механизмы предотвращения

| Тип | Мера | Реализация |
|-----|------|------------|
| **Корректирующие** | Исправить конкретную ошибку | Немедленное исправление в базе |
| **Предупреждающие** | Автоматизация проверок | `test-assertions.ps1`, `GoldenSet.json` |
| **Мониторинг** | Отслеживание источников | Task Scheduler, `audit-session-save.ps1` |
| **Процедуры** | Обновление правил | `before-action-checklist-v2.ps1` |
| **Валидация** | Проверка перед коммитом | `test-documentation.ps1` |
| **Обучение** | Разбор ошибок на встречах | Еженедельный обзор (воскресенье) |

---

### 9.5: Метрики обучения на ошибках

| Метрика | Формула | Цель | Текущее |
|---------|---------|------|---------|
| **Повторные ошибки** | (# повторных) / (# всего) | < 5% | 4-5 за сессию 🔴 |
| **Время исправления** | Σ(время) / (# ошибок) | < 30 мин | 2-3 часа 🔴 |
| **Предотвращение** | (# с правилами) / (# всего) | 100% | ~70% 🟡 |
| **RCA проведено** | (# с RCA) / (# всего) | 100% | ~50% 🟡 |
| **Доверие пользователей** | Динамика оценок | Рост | — |

---

### 9.6: Интеграция в жизненный цикл

**Регулярные мероприятия:**

| Мероприятие | Когда | Ответственный |
|-------------|-------|---------------|
| **Еженедельный обзор ошибок** | Воскресенье, 18:00 | ИИ (автоматически) |
| **Аудит базы знаний** | 1-е число месяца | `full-kb-audit.ps1` |
| **Обновление метрик** | После каждой ошибки | `ERROR_METRICS_DASHBOARD.md` |
| **Постмортем инцидента** | По факту серьёзной ошибки | ИИ + Пользователь |
...
Обнаружена ошибка
       ↓
Записать в ERROR_LOG.md
       ↓
Анализ: «5 почему»
       ↓
Создать правило в ANTI_PATTERNS.md
       ↓
Добавить проверку в PRE_ACTION_CHECKLIST.md
       ↓
Закоммитить
       ↓
Отследить метрики
       ↓
Если повторилась → Усилить правило
```

---

### 9.7: Культура обучения

**Принципы:**

| Принцип | Описание |
|---------|----------|
| **Безопасная среда** | Ошибка — возможность улучшить, а не повод для наказания |
| **Прозрачность** | Информация об ошибках и мерах доступна всем |
| **Системный подход** | Ищем причины в процессах, а не в людях |
| **Поощрение инициатив** | Эффективные предложения поощряются |

---

### 9.8: Инструменты

| Категория | Инструменты | Реализация |
|-----------|-------------|------------|
| **Логирование** | Логи сессий, метрики | `sessions/`, `ERROR_METRICS_DASHBOARD.md` |
| **Трекинг** | Задачи, ошибки | `tasks-dag.json`, `ТЕКУЩАЯ_ЗАДАЧА.md` |
| **Анализ** | «5 почему», диаграммы | `ERROR_LOG.md` (раздел RCA) |
| **Тестирование** | Регрессионные тесты | `test-assertions.ps1`, `GoldenSet.json` |
| **Аудит** | Проверка ссылок, дубликатов | `check-links.ps1`, `find-duplicates.ps1` |
| **Документирование** | Wiki, руководства | `reports/ERROR_LEARNING_GUIDE.md` |

---

### 9.9: Рекомендации к изучению
...
- [ ] Организовать еженедельный разбор ошибок
- [ ] Создать дашборд с метриками качества
- [ ] Настроить авто-создание задач при битых ссылках
...
- [`reports/ERROR_LEARNING_GUIDE.md`](./reports/ERROR_LEARNING_GUIDE.md) — Руководство по обучению
- [`reports/ERROR_METRICS_DASHBOARD.md`](./reports/ERROR_METRICS_DASHBOARD.md) — Метрики

---

**Последнее обновление:** 4 марта 2026 г. (Добавлен Раздел 9: Методология)
